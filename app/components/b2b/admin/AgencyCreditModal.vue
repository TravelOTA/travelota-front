...
watch(() => props.creditLine.status, (val) => { selectedStatus.value = val; });

const mockLedger = ref([
  { id: 1, date: '2026-03-10', description: 'Reserva HB-4921', type: 'charge', amount: -800, balance: 9200 },
  { id: 2, date: '2026-03-15', description: 'Reserva HB-5102', type: 'charge', amount: -1200, balance: 8000 },
  { id: 3, date: '2026-03-18', description: 'Ajuste manual', type: 'deposit', amount: 500, balance: 8500 },
  { id: 4, date: '2026-03-22', description: 'Reserva HB-5310', type: 'charge', amount: -1700, balance: 6800 },
]);
  ...
  <div class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      <div class="bg-gray-50 dark:bg-gray-800/50 rounded-xl p-4 border border-gray-100 dark:border-gray-800 shadow-sm">
        <p class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">{{ t('agency.wallet.credit.limit') }}</p>
        <p class="text-xl font-black text-gray-900 dark:text-white">{{ formatCurrency(creditLine.limit) }}</p>
      </div>
      <div class="bg-gray-50 dark:bg-gray-800/50 rounded-xl p-4 border border-gray-100 dark:border-gray-800 shadow-sm">
        <p class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">{{ t('agency.wallet.credit.used') }}</p>
        <p class="text-xl font-black text-orange-600 dark:text-orange-400">{{ formatCurrency(creditLine.used) }}</p>
      </div>
      <div class="bg-gray-50 dark:bg-gray-800/50 rounded-xl p-4 border border-gray-100 dark:border-gray-800 shadow-sm">
        <p class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">{{ t('agency.wallet.credit.available') }}</p>
        <p class="text-xl font-black text-green-600 dark:text-green-400">{{ formatCurrency(creditLine.available) }}</p>
      </div>
      <div class="bg-gray-50 dark:bg-gray-800/50 rounded-xl p-4 border border-gray-100 dark:border-gray-800 shadow-sm">
        <div class="flex justify-between items-start mb-2">
          <p class="text-xs font-semibold text-gray-500 uppercase tracking-wider">{{ t('agency.wallet.credit.debt') }}</p>
          <UBadge
        ...
        <p class="text-xl font-black" :class="creditLine.debt > 0 ? 'text-red-600 dark:text-red-400' : 'text-gray-900 dark:text-white'">
    ...
    <div>
      ...
      <UTable :data="mockLedger" :columns="ledgerColumns">
        ...
        <template #type-cell="{ row }">
          <UBadge :color="row.original.type === 'charge' ? 'error' : 'success'" variant="soft" size="xs">
    ...
    <div class="flex flex-col gap-8 border-t border-gray-200 dark:border-gray-800 pt-8 mt-4">
      <!-- 3a. Adjust limit and Status -->
      <div class="space-y-6">
        <h4 class="font-bold text-sm flex items-center gap-2">
          <UIcon name="i-heroicons-cog-6-tooth" class="w-4 h-4 text-primary-500" />
          Configuración General
        </h4>
        
        <UFormField :label="t('agency.wallet.credit.newLimit')">
          <div class="flex gap-2">
            <UInput v-model.number="newLimit" type="number" :min="0" class="flex-1" />
            <UButton
              color="primary"
              variant="soft"
        ...
        <UFormField label="Estado de la Línea">
          <div class="flex items-center gap-4">
            ...
            <UButton
              ...
              variant="ghost"
              size="xs"
              :icon="selectedStatus === 'active' ? 'i-heroicons-lock-closed' : 'i-heroicons-lock-open'"
              @click="selectedStatus = (selectedStatus === 'active' ? 'blocked' : 'active'); saveChanges()"
            >
              {{ selectedStatus === 'active' ? 'Bloquear Crédito' : 'Activar Crédito' }}
            </UButton>
          </div>
        </UFormField>
      </div>
      ...
      <div class="space-y-4 bg-gray-50/50 dark:bg-gray-800/20 p-5 rounded-2xl border border-gray-200 dark:border-gray-800">
        <h4 class="font-bold text-sm flex items-center gap-2">
          <UIcon name="i-heroicons-arrows-right-left" class="w-4 h-4 text-primary-500" />
          Ajuste Manual / Pago
        </h4>
        <p class="text-xs text-gray-500">
          Usa este formulario para registrar pagos recibidos o ajustes de saldo manuales.
        ...
        <div class="space-y-3">
          <UFormField label="Monto del Ajuste (USD)">
            <UInput v-model.number="adjustmentAmount" type="number" placeholder="Ej: 500 o -500" />
          </UFormField>
          <UFormField label="Nota / Concepto">
            <UInput v-model="adjustmentNote" placeholder="Ej: Pago transferencia BBVA..." />
          </UFormField>
          <UButton
            block
            color="primary"
