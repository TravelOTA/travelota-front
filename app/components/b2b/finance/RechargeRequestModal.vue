      ...
      <UCard>
        ...
        <div v-else class="space-y-5">
          ...
          <div class="rounded-lg border border-gray-200 dark:border-gray-700 divide-y divide-gray-100 dark:divide-gray-800 overflow-hidden">
            ...
            <div
              v-for="field in [
                { key: 'bank', label: t('agency.wallet.recharge.bank.bankName'), value: BANK_DETAILS.bank, copy: false },
                { key: 'holder', label: t('agency.wallet.recharge.bank.holder'), value: BANK_DETAILS.holder, copy: false },
                { key: 'iban', label: t('agency.wallet.recharge.bank.iban'), value: BANK_DETAILS.iban, copy: true },
                { key: 'bic', label: t('agency.wallet.recharge.bank.bic'), value: BANK_DETAILS.bic, copy: true },
                { key: 'concept', label: t('agency.wallet.recharge.bank.concept'), value: rechargeReference, copy: true },
              ]"
              :key="field.key"
              class="flex items-center justify-between px-4 py-3 gap-4"
            >
              <span class="text-xs text-gray-500 shrink-0">{{ field.label }}</span>
              <div class="flex items-center gap-2 min-w-0">
                <span
                  class="text-sm font-semibold text-gray-900 dark:text-white font-mono truncate"
                  :class="{ 'text-primary-600 dark:text-primary-400': field.key === 'concept' }"
                >
                  {{ field.value }}
                </span>
                <UButton
          ...
          <div class="rounded-lg bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 p-4 text-center">
            ...
            <p class="mt-1 text-2xl font-extrabold text-green-700 dark:text-green-400">
              ${{ amount?.toLocaleString() }}
            </p>
          </div>

          <p class="text-xs text-gray-400 text-center">
            {{ t("agency.wallet.recharge.bank.conceptWarning", { concept: rechargeReference }) }}
          </p>
